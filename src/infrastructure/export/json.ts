import { db, dbHelpers } from '@/infrastructure/storage/dexie/db'
import { supabaseSyncService } from '@/infrastructure/supabase/db_service'
import type { ProjectExportData, Project, Task, Dependency, Resource, Milestone } from '@/domain/models'
...
export function validateProjectImport(data: unknown): ProjectExportData | null {
  ...
  if (!exportData.version) {
    exportData.version = CURRENT_VERSION
  }

  if (!exportData.taskResourceAssignments && exportData.assignments) {
    exportData.taskResourceAssignments = exportData.assignments
  }
...
export async function importProject(data: ProjectExportData): Promise<string> {
  ...
  try {
    await supabaseSyncService.saveProjectToCloud(project.id, { project, tasks, dependencies, milestones }, 1)
    const allLocalResources = await db.resources.toArray()
    await supabaseSyncService.syncResourcesToCloud(allLocalResources)
  } catch (cloudErr) {
    console.warn('Cloud sync of imported project skipped:', cloudErr)
  }
